...
router.post('/changeMainInfo', auth,  async (req, res) => {
    try {
        ...
        await User.updateOne({ _id: userId }, { name: req.body.name, dateOfBirth: req.body.dateOfBirth, about: req.body.about }, (error, data) => {
            if (error) {
                console.log(error);
                return next(error);
            } 
...
router.post('/changeSettingsPassword', auth,  async (req, res) => {
    try {
        ...
        if(isMatch) {
            ...
            await User.updateOne({ _id: userId }, { password: hashedPassword }, (error, data) => {
                if (error) {
                    console.log(error);
                    return next(error);
                } 
